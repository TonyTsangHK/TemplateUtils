 ...
 * Time: 17:58
 */
class MapValueSubstitutor(private val valueMap: Map<String, Any>) : ValueSubstitutor {
    ...
    override fun <V> getVariableValue(key: String, nullSafe: Boolean, vararg substitutes: String): V? {
        return getVariableValue<V>(key.split(".").dropLastWhile { it.isEmpty() }.toTypedArray(), nullSafe, *substitutes)
    }
    ...
    override fun <V> getVariableValue(key: String, vararg substitutes: String): V? {
        return getVariableValue<V>(key.split(".").dropLastWhile { it.isEmpty() }.toTypedArray(), *substitutes)
    }
